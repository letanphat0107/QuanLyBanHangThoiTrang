...
public class ManHinh_QA_KichThuoc extends javax.swing.JPanel {
    ...
    public void xuLyXoaKichThuoc() {
        ...
        if (row != -1) {
            if (JOptionPane.showConfirmDialog(this, "Bạn có chắc là xóa dòng này không?", "Cảnh Báo", JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
                String maKichThuoc = txt_MaKichThuoc.getText();
    ...
    public void xuLyCapNhatKichThuoc() {
        ...
        int row = tbl_KichThuoc.getSelectedRow();
        if (row != -1) {
            dao_KichThuoc.catNhatDLKichThuoc(kichThuoc);
            for (int i = 0; i < tbl_KichThuoc.getRowCount(); i++) {
                String maKichThuocTable = tbl_KichThuoc.getValueAt(row, 0).toString();
                if (maKichThuocTable.equalsIgnoreCase(maKichThuoc)) {
    ...
    public void xuLyTimKiemKichThuoc() {
        String tuKhoaMaKichThuoc = txt_MaKichThuoc.getText();
        ...
        } else if (tuKhoaMaKichThuoc.equals("")) {
